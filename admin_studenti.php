<?php
session_start();
require("config.php");

$str='stud';
$aStud="class='trenutna'";
$naslov="apliTest | Studenti";

require("header.php");
include("register/admin_reg.php");
echo"
<h1>Lista Studenata</h1>

<table class='adminList'>
<tr>
	<th>No</th>
	<th>Ime</th>
	<th>Prezime</th>
	<th>Indeks</th>
	<th>Email</th>
	<th>Smer</th>
	<th>Tip</th>
	<th>Izbriši</th>
</tr>
";

$sqlStud="SELECT * FROM profil WHERE tip='student' ORDER BY smer";
$run_sqlStud= mysqli_query($con, $sqlStud);
$i=1;
while($row_stud = mysqli_fetch_array($run_sqlStud)){
	$id=$row_stud['id'];
	$ime=$row_stud['ime'];
	$prezime=$row_stud['prezime'];
	$smer=$row_stud['smer'];
	$index=$row_stud['br_indeksa'];
	$mail=$row_stud['email'];
	$tip=$row_stud['tip'];
	echo"
		<tr>
			<td>$i</td>
			<td>$ime</td>
			<td>$prezime</td>
			<td>$index</td>
			<td>$mail</td>
		<form name='frmSmer' method='post' action='promSmer.php'>
			<td style='width:250px;'>
				<select name='pormSmer' style='width:70%;'>";
					echo"<option value='' style='color:gray;' ";
				if($smer==0){
						echo" disabled selected";
					}
					echo">Izaberite smer</option>";
				$query=" SELECT * FROM smer";
				$result=mysqli_query($con, $query);
				while($row=mysqli_fetch_array($result)){
					$idSmera=$row['id'];
					$nazivSmera=$row['naziv'];
		
					echo"<option value='$idSmera'";
						  if($idSmera==$smer){
							  echo"selected";
							}
							  echo">
							$nazivSmera</option>";
				}
			echo"
				</select>
				<input type='hidden' name='id' value='$id'>
				<input type='submit' name='proSmer' value='OK'   style='margin-left:10px;'>
			</td>
		</form>
		<form name='frmTip' method='post' action='promTip.php'>
			<td style='width:150px;'>
				<select name='tip'>
					<option value='student' selected>Student</option>
					<option value='profesor'>Profesor</option>
					<option value='admin'>Admin</option>
				</select>
				
				<input type='hidden' name='id' value='$id'>
				<input type='submit' name='promTip' value='OK'   style='margin-left:10px;'>
			</td>
		</form>
			<td style='padding-left:20px;'><a style='color:red; '  href='delStudent.php?id=$id&tip=$tip'>[X]</a></td>
		</tr>
	";
	$i++;
}


echo"
</table>
";
//===============================================================



require("footer.php");

?>